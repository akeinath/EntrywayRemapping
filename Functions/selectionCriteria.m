...
%                 allMasks{doorA+length(s.processed.roi.door(1,:))} = [isMostRecent(doorA,isInRoom) & ~(h2<h2(end)./2)];
            ...
            nT = s.processed.trace;
%             nT = bsxfun(@rdivide,nT,nanmax(nT,[],2));
%             nT = nT - 0.2;
%             nT(nT<0) = 0;
%             nT = nT<=0.2;
            [a b c ivals] = getMatchedMapsNMasks(s.processed.p(:,isInRoom),nT(:,isInRoom),allMasks);
...
%                 nanmax(s.processed.splithalf.hallwayXdoors.vals,[],2)',tmp(:)))';
        ...
        if ismember({'hallwayxdoor2'},varargin)
            %%% In-room split half reliability, include doorway
            ...
            didChange = true;
            [isIn isMostRecent indexSinceIn] = isInROI(s.processed.p,s.processed.roi.door);
            isInRoom = inpolygon(s.processed.p(1,:)',s.processed.p(2,:)',...
                s.processed.roi.hallway(:,1),s.processed.roi.hallway(:,2));
            ...
            allComp = sort(allComp,2,'descend');
            actual = allComp;
            s.processed.splithalf.hallwayxdoors2.vals = allComp;
            ...
            fprintf([num2str(durat) ' sec']);
            null = sort(null,2,'descend');
            s.processed.splithalf.hallwayXdoors2.null = null;    
            tmp = nanmax(null,[],2);
%             s.processed.splithalf.hallwayXdoors2.p = 1-nanmean(bsxfun(@gt,...
%                 nanmax(s.processed.splithalf.hallwayXdoors2.vals,[],2)',tmp(:)))';
            s.processed.splithalf.hallwayXdoors2.p = 1-nanmean(bsxfun(@gt,...
                nanmax(s.processed.splithalf.hallwayXdoors2.vals,[],2)',permute(tmp,[3 1 2])))';

            figure(1)
            set(gcf,'position',[50 50 250 250])
            cumHist(s.processed.splithalf.hallwayXdoors2.p,[0:0.01:1]);
            hold on
            plot([0 1],[0 1],'color',[0.5 0.5 0.5],'linestyle',':','linewidth',1)
            ylabel('Cumulative Proportion')
            xlabel('P-Value')
            ...
            fprintf('\n\t\tReliable spatial cells:  %i of %i (%0.2f%%)\n',[nansum(s.processed.splithalf.hallwayXdoors2.p <= 0.05) ...
                length(s.processed.splithalf.hallwayXdoors2.vals(:,1)) nanmean(s.processed.splithalf.hallwayXdoors2.p <= 0.05).*100]);
        end
