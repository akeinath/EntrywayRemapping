...
%     groupColor = tmp(round(linspace(1,length(tmp),groups)),:);
    ...
    groupColor = [0.8 0.8 0.8; 0.3 0.3 0.9; 0.2 0.2 0.2; 0.5 0.5 0.9; 0.7 0.7 0.9; ...
...
%     groupColor = [colors(([1 40 160 200]),:); 0 0 0 ];
    ...
    for i = 1:groups
        for j = 1:ticks
            jitters{i,j} = randn(size(d{i,j})).*0.0;
        end
    end
    for i = 1:groups
        for j = 1:ticks
            ...
            if length(d{i,j})<=100
                plot(j-(w./2)+(wpg.*(i-1))+wpg./2+jitters{i,j},d{i,j},'linestyle','none','color',dotColor,...
                ...
                if i < groups && length(d{i,j}) == length(d{i+1,j}) && length(d(:,1)) > 1
                    plot([repmat([j-(w./2)+(wpg.*(i-1))+wpg./2 ...
                        ...
                        [d{i,j} d{i+1,j}]','color',dotColor,'linewidth',0.8)
                end
            end
...
%             d{i,j}(d{i,j}>nanmean(d{i,j})+2.*nanstd(d{i,j})) = [];
            ...
            if length(d{i,j})>1
                plot(repmat(nanmean([(j-(w./2))+(wpg.*(i-1)) (j-(w./2))+(wpg.*(i))]),[1 2]),...
                    [nanmean(d{i,j}) nanmean(d{i,j})+dir.*nanstd(d{i,j})./sqrt(sum(~isnan(d{i,j})))],'color',edgeColor,'linewidth',1.5);
    %             plot([nanmean([(j-(w./2))+(wpg.*(i-1)) (j-(w./2))+(wpg.*(i))])-(wpg./ebW) ...
    %                 nanmean([(j-(w./2))+(wpg.*(i-1)) (j-(w./2))+(wpg.*(i))])+(wpg./ebW)],...
    %                 [nanmean(d{i,j})+dir.*nanstd(d{i,j})./sqrt(numel(d{i,j})) ...
    %                 nanmean(d{i,j})+dir.*nanstd(d{i,j})./sqrt(numel(d{i,j}))],'color','k','linewidth',2)
                plot(repmat(nanmean([(j-(w./2))+(wpg.*(i-1)) (j-(w./2))+(wpg.*(i))]),[1 2]),...
                    [nanmean(d{i,j}) nanmean(d{i,j})-dir.*nanstd(d{i,j})./sqrt(sum(~isnan(d{i,j})))],'color',edgeColor,'linewidth',1.5);
    %             plot([nanmean([(j-(w./2))+(wpg.*(i-1)) (j-(w./2))+(wpg.*(i))])-(wpg./ebW) ...
    %                 nanmean([(j-(w./2))+(wpg.*(i-1)) (j-(w./2))+(wpg.*(i))])+(wpg./ebW)],...
