...
%     end
    ...
    colors = [{[0.8 0.8 0.8; 0.5 0.5 0.5]} {[0.5 0.5 0.9; 0.5 0.9 0.9; 0.2 0.2 0.2]}];
    for ai = 1:length(d(1,:))
                ...
                if length(curD{i})>1
                    ...
                    for j = 1:length(cts2)
                        ...
                        h2(j) = patch('Faces',[1:length([start+cts(i).*start2:degStep:start+cts(i).*nansum(cts2(1:j))]).*2],...
                            ...
                            radii(3).*sin(2.*pi.*[start+cts(i).*nansum(cts2(1:j)):-degStep:start+cts(i).*start2])]'],'FaceColor',colors{2}(j,:),...
                            'edgecolor','w');
                    end
                else
...
%             legend([h1 h2],[labels{1} labels{2}],'box','off','location','northwest');
            totes = 0;
            for i = 1:length(d{coni,ai}(1,:))
                totes = totes + nansum(nansum(cat(1,d{coni,ai}{:,i})));
            end
            text(0,0,num2str(totes),'horizontalalignment','center','verticalalignment','middle')
        end
    end
end
