...
%             s.processed.p = imfilter(s.processed.p,fspecial('gauss',[1 15],2),'same','replicate');
            ...
            if isfield(s.processed,'exclude')
                ...
                inds = (s.processed.splithalf.roomXdoors.p<=pThresh);% & ~s.processed.exclude.SFPs);
%                 inds = (s.processed.splithalf.wholemap_si.p<=0.01);% & ~s.processed.exclude.SFPs);
...
% % %                 q2 = false(4);
% % %                 q2(1:2,3:4) = true;
% % %                 delayVals = nan(2,length(delayBins),length(gT(:,1)));
% % %                 for q = 1:length(delayBins)
% % %                     [blah blah2 mfr] = getMatchedSamplingValues(s.processed.p(:,isInRoom), ...
% % %                         gT(:,isInRoom),allDelayMasks(:,q)',q2);
% % %                     if all(isnan(mfr(:)))
% % %                         continue
% % %                     end
% % %                     delayVals(:,q,:) = permute([help_getMaskedVals(mfr(1:2,3:4,:),[true false; false true]) ...
% % %                         help_getMaskedVals(mfr(1:2,3:4,:),[false true; true false])],[3 2 1]);
% % %                 end
...
% % %                         continue
% % %                     end
% % %                     delayVals(:,q) = [help_getMaskedVals(allComp(1:2,3:4),[true false; false true]) ...
% % %                         help_getMaskedVals(allComp(1:2,3:4),[false true; true false])];
% % %                 end
% % %                 crossAllCompDelayed{group,mi} = cat(3,crossAllCompDelayed{group,mi},delayVals);
...
% % %                 delayVals = nan(2,length(delayBins),length(gT(:,1)));
% % %                 for q = 1:length(delayBins)
% % %                     delayVals(:,q,:) = permute([help_getMaskedVals(mfr(4.*(q-1)+[1:2],4.*(q-1)+[3:4],:),[true false; false true]) ...
% % %                         help_getMaskedVals(mfr(4.*(q-1)+[1:2],4.*(q-1)+[3:4],:),[false true; true false])],[3 2 1]);
% % %                 end
% % % 
% % %                 crossAllCompDelayed{group,mi} = cat(3,crossAllCompDelayed{group,mi},delayVals);
...
%             if doPrediction
%                 [map samp allComp] = getMatchedPrediction(s.processed.p(:,isInRoom), ...
%                     gT(:,isInRoom),[{isMostRecent(1,isInRoom)} {isMostRecent(2,isInRoom)}]);
%             end
...
%                 [a b] = nanmax(isMostRecent,[],1);
%                 [c d] = nanmax(isSecondMostRecent,[],1);
%                 dd = (b-1).*2+d;
%                 dd(a==0|c==0) = 0;
%                 queryMask = false(8);
%                 queryMask(1:4,6:8) = true;
%                 allMasks = repmat({[]},[1 8]);
%                 for di = 1:4
%                     allMasks{di} = dd(isInRoom)==di & half(isInRoom);
%                     allMasks{di+4} = dd(isInRoom)==di & ~half(isInRoom);
%                 end
%                 
%                 [map samp allComp] = getMatchedMapsNMasks(s.processed.p(:,isInRoom), ...
%                     gT(:,isInRoom),allMasks,queryMask);
            end
...
% % %                 outP = ['Plots/HallwayDirectionMaps/' paths{isM(si)}(slashInds+1:end-4)];
